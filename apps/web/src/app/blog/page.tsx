  ...
  return (
    <>
      ...
      <main className="container py-16 md:py-24">
        ...
        {posts.length === 0 && (
          <p className="mt-12 text-white/40 text-sm">No posts yet. Check back soon!</p>
        )}
        ...
        <div className="mt-8 grid md:grid-cols-3 gap-6">
          {posts.map((p) => (
            <a
              key={p.id}
              href={`/blog/${p.slug}`}
              className="card overflow-hidden block hover:opacity-90 transition-opacity"
            >
              {p.imageUrl && (
              ...
              <div className="p-5">
                <h2 className="text-xl font-[var(--font-serif)]">{p.title}</h2>
